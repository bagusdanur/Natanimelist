        ...
        <div>
            ...
            {openMenu && (
                <div ref={menuRef} className='dropdownMenu z-10'>
                    {isLoggedIn ? (
                        <React.Fragment>
                            <h3 className='text-color-titleColor flex flex-row gap-1'><User size={22} /> {user.name}</h3> {/* Menggunakan user.name karena prop user tampaknya memiliki properti name dan email */}
                            <p className='pb-3 text-base text-color-titleColor flex flex-row gap-1'><Envelope size={22} /> {user.email}</p>
